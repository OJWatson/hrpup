...
scenario_maps <- list("scenarios" = scenarios,
                      "map_data" = map_data,
                      "map" = world_map)
saveRDS(scenario_maps, "analysis/data_derived/scenario_maps_full.rds")
scenario_maps <- readRDS("analysis/data_derived/scenario_maps_full.rds")

# now just save the t
for(i in seq_along(scenario_maps$map_data)) {
  scenario_maps$map_data[[i]] <- scenario_maps$map_data[[i]] %>%
    select(id_1, t)
}
scenario_maps <- list("scenarios" = scenario_maps$scenarios,
