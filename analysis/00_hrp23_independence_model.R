...
hrpdat <- hrpdat %>%
  ...
  mutate(prev = replace(prev, prev <= 0, 0.00001))

hrpdat$continent <- as.factor(countrycode::countrycode(final$ISO_ctry,"iso2c", "continent"))
...
gg_23_coocc <- final2 %>%
  ...
  geom_point() +
  scale_x_log10(limits = c(0.00001, 1), labels = scales::percent) +
...
glm(cbind(WHO_HRP2_HRP3_DELETIONS_POS_N, WHO_HRP2_DELETION_TESTED_N - WHO_HRP2_HRP3_DELETIONS_POS_N) ~ (prev),
